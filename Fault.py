...
def process_frame(frame):
    ...
    line1_x = target_width // 2 - 20
    line2_x = target_width // 2 + 20
    cv2.line(frame, (line1_x, 0), (line1_x, target_height), (0, 255, 255), 2)
    cv2.line(frame, (line2_x, 0), (line2_x, target_height), (0, 255, 255), 2)
    ...
    for obj in tracked_objects:
        ...
        if tracker.center_points[obj_id]['last_position'] is not None:
            ...
            if last_position <= line1_x and center_x > line1_x:
                walk_in_count += 1
                print(f"Walk In Count: {walk_in_count}")
            elif last_position >= line2_x and center_x < line2_x:
                walk_out_count += 1
                print(f"Walk Out Count: {walk_out_count}")

        tracker.center_points[obj_id]['last_position'] = center_x

        person_bbox = (x, y, x + w, y + h)
        violating_uniform = True

        for ux1, uy1, ux2, uy2 in uniform_rects:
            uniform_bbox = (ux1, uy1, ux2, uy2)
            if is_overlapping(person_bbox, uniform_bbox):
                violating_uniform = False
                break
...
@app.route('/')
def index():
    videos = ['video/test1.mp4', 'video/test2.mp4']
